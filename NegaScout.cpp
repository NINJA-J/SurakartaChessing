...
double CNegaScout::NegaScout_ABTree(int depth, bool isBlackPlay, BV_TYPE alpha, BV_TYPE beta) {
	...
	for (int i = 0; i < count; i++) {
		...
		BV_TYPE t = isMax ?
			NegaScout_ABTree(depth - 1, isBlackPlay, best, MAX_VALUE) :
			NegaScout_ABTree(depth - 1, isBlackPlay, MIN_VALUE, best);
		chessBoard.unMove();
...
double CNegaScout::negaScoutMinWin(ChessBoard chessBoard, int depth, bool isBlackPlay, BV_TYPE alpha, BV_TYPE beta) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (depth == 3) {
			...
			CString temp;
			temp.Format(
				"走棋方%d，状态%lld,(%d,%d)->(%d,%d)", isBlackPlay,chessBoard.getId(),moveList[i].From.x, moveList[i].From.y, moveList[i].To.x, moveList[i].To.y);
			AfxMessageBox(temp);
		}
		BV_TYPE t = isMax ?
...
double CNegaScout::threadMain()
{
	...
	//	CString temp;
	//	temp.Format(
	//	"子树分数%lf,状态%lld",otherScore,chess.getId());
	//	AfxMessageBox(temp);
		//k++;
