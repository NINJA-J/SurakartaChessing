...
CNegaScout::CNegaScout(Task & t):chessBoard(t) {
	...
	*/
	...
	//chessBoard.setChessPosition(t.position, t.isBlackTurn);
}
 ...
 */
	...
	/*CString temp;
	temp.Format(
		"Totle Time:%lfS,走法：(%d,%d)->(%d,%d)\n分数：%f",
		(double)(endTime - startTime) / CLOCKS_PER_SEC, bestMove.From.x, bestMove.From.y, bestMove.To.x, bestMove.To.y, score
	);
	AfxMessageBox(temp);*/
	return bestMove;
}
...
inline BV_TYPE CNegaScout::negaScoutSearch(Task &t) {
	/*CString temp;
	temp.Format(
		"进来了%lf,%lf,%d", (double)t.alpha, (double)t.beta, t.depth);
	AfxMessageBox(temp);*/
	switch (t.useMethod) {
...
BV_TYPE CNegaScout::negaScoutAlphaBeta(int depth, BV_TYPE alpha, BV_TYPE beta) {
	...
	if (depth <= 0)
#ifdef USE_NEW_ABTREE
		return (-m_pEval.evaluate(chessBoard, isBlackPlay))*(SEARCH_DEPTH % 2 ? 1.0 : -1.0);
#else
...
//本处是基于负极大值的alpha-beta剪枝算法，为什么此时遇见了beta会剪枝，因为我们将最后一步的估值计算为走棋方
//的优势估值“并取负”，所以上一层应该取最大值，所以为alpha结点，判断剪枝的结点是beta结点
		if (score > alpha) {
			alpha = score;
			if (depth == m_nMaxDepth)
				bestMove = moveList[i];
...
BV_TYPE CNegaScout::negaScoutMinWin(int depth, BV_TYPE alpha, BV_TYPE beta) {
	...
	if (depth <= 0) 
		return m_pEval.addBoardValue(
			chessBoard.getId(), 0, m_pEval.evaluate(chessBoard, isBlackPlay)
		);
...
BV_TYPE CNegaScout::negaScoutPVSProc(int depth, BV_TYPE alpha, BV_TYPE beta) {
	CHESSMOVE moveList[200];
	BV_TYPE value, delta = m_pEval.getValueDelta();
	//BYTE position[6][6];
	if (negaScoutPrevTest(depth, value)) return value;
